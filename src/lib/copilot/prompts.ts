...
export function leakProofCopilotSystemInstruction(): string {
  return [
    ...
    'Feature-specific routes that persist drafts, critiques, hierarchy relationships, reports, emails, invoices, approvals, or customer-ready state require explicit human confirmation outside the read-only answer.',
...
export function buildCopilotGeminiPrompt(input: {
  ...
  return [
    ...
    'Use answer_type as one of: audit_summary, direct_answer, finding_explanation, evidence_review, report_readiness, missing_data, data_mapping, audit_readiness, next_best_action, false_positive_risk, reviewer_checklist, cfo_summary, recovery_note, contract_hierarchy, root_cause, prevention_recommendations.',
    ...
    'For data mapping, missing data, audit readiness, next best action, evidence quality, false-positive risk, contract hierarchy, recovery note, CFO summary, root cause, and prevention requests, explain the provided advisory tool output. Do not add new facts.',
    '',
    JSON.stringify({
